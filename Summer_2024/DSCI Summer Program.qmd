...
ggplot(semesterv2, aes(x = `Semester Number at Xavier`,)) +
  ...
  labs(title = "Enrollment in Core Classes by Semester", y = 'Percentage of Core Courses (%)') +
  theme_bw() +
...
for (X in indiv.courses)
{
  ...
  print(ggplot(temp,aes(x = `Semester Number at Xavier`))+
          ...
          labs(title = paste("Enrollment in",X,"Course by Semester"), y = "Percentage of Courses (%)"))
}
```
...
Data %>%
  ...
  labs(title="Timing of Core Courses, 2015-2023 (All Students)", x ="Semester", y = "Core Curriculum Requirement") +
...
ggplot(combined_data, aes(x = as.numeric(`Semester Number at Xavier`), y = reorder(Requirement, as.numeric(`Semester Number at Xavier`), mean))) +
  ...
  theme_minimal() +
  labs(title = "Timing of Core Courses 2015-2023", x = "Semesters", y = "Core Curriculum") +
...
ggplot(nf_boxplots, aes(x = as.numeric(`Semester Number at Xavier`), y = reorder(Requirement, as.numeric(`Semester Number at Xavier`), mean))) +
  geom_boxplot(fill = "light blue", color = "black") +
  theme_minimal() +
  labs(title = "Timing of Core Courses for New Freshman (NF) Students 2015-2023", x = "Semesters", y = "Core Curriculum") +
  facet_wrap(~ Group, scales = "free_y")
```
...
ggplot(nt_boxplots, aes(x = as.numeric(`Semester Number at Xavier`), y = reorder(Requirement, as.numeric(`Semester Number at Xavier`), mean))) +
  geom_boxplot(fill = "light blue", color = "black") +
  theme_minimal() +
  labs(title = "Timing of Core Courses for New Transfer (NT) Students 2015-2023", x = "Semesters", y = "Core Curriculum") +
  facet_wrap(~ Group, scales = "free_y")
```
...
ggplot(data = total_langcredit, aes(x = total_value))+
  geom_bar(fill = "#1A1AFF", color = "black")+
  scale_x_continuous(breaks = seq(0,21, by =3))+
  geom_text(stat = 'count', aes(label= after_stat(count)), vjust = -.5)+
  labs(title = "Total Language Requirement Credits Earned", x = "Earned Credits", y = "Count of Students")+
...
ggplot(data = combination, aes(x = total_rows))+
  ...
  geom_bar(fill = "#D7D3CC", color = "black")+

  scale_x_discrete(breaks = seq(0,10, by =1))+

  geom_text(stat = 'count', aes(label= after_stat(count)), vjust = -.5)+

  labs(title = "Total Language Requirement Courses Attempted by Student", x = "Number of Attempted Language Courses", y = "Count of Students")+
...
Data %>% 
  ...
  labs(title = "Letter Grades of Students in All Core Classes", y = 'Percentage of Grades (%)')
...
data2 <- read_excel("Copy of RetainedStudents2015-2021.xlsx") %>% 
...
result %>% 
  ...
  summarize(count = n(), .groups='drop') %>%
  mutate(percentage=count/Total.finished*100) %>% #6391 is the total number of students who graduated from Xavier (with 4 years)
  ...
  geom_text(aes(label=paste(round(percentage,1),"%") , hjust = -0.25))+
  coord_flip() +
...
ggplot(major_grade_summary, aes(x = Grade, y = Percentage, fill = Grade)) +
  ...
  labs(title = "Letter Grades of Students by Major Category",
